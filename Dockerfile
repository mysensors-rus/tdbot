...
RUN apt-get update && apt-get install -y git build-essential cmake libssl-dev liblua5.2-dev gperf libconfig++-dev  zlib1g-dev ccache && \
    git clone --recursive https://github.com/mysensors-rus/tdbot.git && cd ./tdbot && git submodule update --remote --merge && \
    cd .. && mkdir tdbot-build && cd tdbot-build &&  cmake -DCMAKE_BUILD_TYPE=Release ../tdbot && make telegram-bot && make install && \
